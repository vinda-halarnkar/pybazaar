...
class Command(BaseCommand):
        ...
        try:
            ...
            while skip < TOTAL_PRODUCTS:
                ...
                imported_categories = {
                    ...
                    for category in Category.objects.filter(category_slug__in=categories)
                }
                imported_statuses = {
                    status.status_name: status
                    for status in AvailabilityStatus.objects.filter(
                        status_name__in=availability_statuses
                    )
                }
                imported_brands = {
                    brand.brand_name: brand for brand in Brand.objects.filter(brand_name__in=brands)
                }
                ...
                for product in products:
                    ...
                    product_images = [{"image_path": product["thumbnail"], "is_thumbnail": True}]
                    if "images" in product:
                        for image_path in product["images"]:
                            product_images.append({"image_path": image_path, "is_thumbnail": False})
    ...
    def import_categories(self):
        try:
            ...
            existing_categories = set(
                Category.objects.filter(category_slug__in=category_slugs).values_list(
                    "category_slug", flat=True
                )
            )
    ...
    def import_brands(self, brands):
        try:
            existing_brands = set(
                Brand.objects.filter(brand_name__in=brands).values_list("brand_name", flat=True)
            )
            new_brands = [
                Brand(brand_name=brand) for brand in brands if brand not in existing_brands
            ]
    ...
    def import_availability_statuses(self, statuses):
        try:
            existing_statuses = AvailabilityStatus.objects.filter(
                status_name__in=statuses
                    ...
                    with transaction.atomic():
                        ...
                        for created_product in created_products:
                            product_images_instances.extend(
                                [
                                    ProductImage(product=created_product, **image)
                                    for image in product_images[created_product.sku]
                                ]
                            )
